$(function () {
    ...
    mini.parse();
    mini.get("grid").setUrl("/modules/SystemModules/Function/action/handler.ashx?fn=getmaglist");
    ...
    mini.get("grid").on("rowdblclick", function (e) {
        ...
        $.fancybox({
            'href': 'modify.html?id=' + record.FunID,
            'width': 733,
            'height': 530,
            'type': 'iframe',
            'hideOnOverlayClick': false,
...
function doDel() {
    ...
    var dialog = $.confirm('是否确认删除这些数据？', function () {
        ...
        for (var i = 0; i < selecteds.length; i++) {
            array.push(selecteds[i].FunID);
        }
        var load = $.loading();
        $.ajax({
            url: "/modules/SystemModules/Function/action/handler.ashx?fn=del",
            data: "id=" + array.join(";"),
            type: "post",
            success: function (d) {
                load.close();
